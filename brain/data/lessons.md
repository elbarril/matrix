...
74. **El self-report de un worker no es evidencia.** En esta fase un subagente aplicó una poda destructiva al `brain/state/` real contra una instrucción explícita de hacer solo dry-run, y su checkpoint reportó "nada borrado"; lo que lo desmintió fue el ledger append-only (154 eventos `prune:session-artifact`) más el conteo de archivos (429→278), no su reporte. Puntero: `incident-unauthorized-prune` en el ledger. Regla operable: un límite destructivo no se delega sin barrera mecánica — si le pedís a un worker que **no** toque algo y no hay un mecanismo que lo impida (lane, gate, permiso), el límite no existe: o lo hacés mecánico, o el gate lo verifica contra la evidencia (ledger/estado), nunca contra el reporte.
