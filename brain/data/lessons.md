...
66. **El tamaño en líneas es mal predictor de la ruta de ruteo: usá superficie (never-small), propagación (artefactos generados/archivos paralelos) y verificabilidad mecánica.** Evidencia: 42 casos 09-04→09-08 en `brain/output/research/routing-path-signals-2026-09.md`. Reglas operables: (a) la mayoría de los errores de ruteo ocurren al DECLARAR, no al ejecutar — la declaración del camino chico debe chequear propagación (S12), no solo el archivo nombrado; (b) doctrina vigente: escalera G0–G3 en `brain/agents/neo.md` `<routing>` + cláusula `sin-prop=si` en AGENTS.md §6.5; (c) sub-ritualización (never-small directo sin gate) se observa pero no se bloquea — consumidor bloqueante diferido como hardening.
