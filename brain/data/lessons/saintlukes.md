...
## Gotchas verificados (2026-09-21)

- **Un fix que vive en un branch que no es el que se deploya se pierde en el próximo sisifo sync.** El fix de preview de adjuntos (`9164a1e`, branch `T1272545_saintlukes_FixAttachmentPreview`) estaba desplegado y andando el 2026-09-02, y el deploy del branch de Event del 2026-09-16 lo pisó (archivo desplegado volvió a la versión pre-fix). Regla: antes de dar por roto un fix ya verificado, chequear si sigue en el branch desde el que se sincroniza la instancia. Detalle/evidencia: `brain/output/saintlukes/eval/t1272545-attachment-preview-regression-2026-09-21.md`.
- **`attachment.url` puede venir vacío (PDFs con preview `unsupported`)**: el link Preview/Open de `attachmentsManager.tpt` queda con `href=""` → el browser lo resuelve a la URL actual y el iframe del preview renderiza la página del portal. Fallback correcto: `attachment.preview.imgPreview` (endpoint `attachmentMultipleImagePreview`). Mismo artefacto de arriba.
