...
| `dispatch` | Hand a validated event to *something* that can do project work unattended, and get back a handle to check on it later. | Prefer for unattended project work; returns a handle for later status checks. |
| `single-flight-acquire` | Claim exclusive execution rights for one registered project's queue lane. | Cheap local filesystem lock; acquire before dispatch. |
| `single-flight-release` | Release exclusive execution rights for one registered project's queue lane. | Must pair with `single-flight-acquire`; release only after ack or terminal state. |
