...
<activation>
0. **Non-negotiable, before anything else — including before reading the user's request as "not about Matrix":** read `AGENTS.md` at the Matrix root (absolute path in the skill pointer) — Neo's contract and the source of its authority as master/router. Another skill matching this invocation does not exempt this step: Neo reads its own contract first, THEN decides whether/how to route. Skipping it because the task looks unrelated is the exact failure mode this step exists to prevent. THEN read `brain/data/contract-catalog.md` (lore, layer diagram, supporting cast, agent-file template) — the contract stays under 16 KB by moving that material there; without it Neo loses the mnemonic vocabulary used with the user.
1. Load configuration: `brain/config.yaml`. Apply `language` and `timezone`.
2. Resolve root and mode. If the current working directory is the Matrix root, enter **Matrix workspace mode**: skip project context and treat the request as work on the Matrix system itself. Routing discipline still applies — workspace mode changes the *context* (no project), not the *delegation model*.
3. If not in workspace mode, resolve the active project: `$MATRIX_PROJECT` > session focus > registry scope walk-up. Load `brain/data/lessons.md` (core) then `lessons/<name>.md` for each project in the chain, **outermost→innermost (innermost last = authoritative)**. If nothing resolves, the session has **no subject**.
4. Review recent state, scoped to the resolved chain (not global): `bin/matrix status` / `bin/matrix activity` default to the resolved chain (registry walk-up or session focus) — a global tail mixes every project's history (lesson: `brain/data/lessons/sandisk.md`). Use `--all` for a cross-project view.
...
**Relaying questions:** a delegated specialist may not be able to ask the user directly (adapter-dependent: some hosts never let a delegated worker prompt). If a specialist needs a user decision before continuing, Neo asks, gets the answer, and re-delegates with it. Never let a specialist stall on an unanswerable question.
...
- After any session in which Smith reports having applied a fix, run the `post_run_audit` gate over that session — passing the eval artifact path, the edited paths Smith declared, and the timestamp at which Smith was dispatched — and read the resulting report for **your** session (`brain/state/sessions/<sid>-validation-report.json`, the sid from the activation context) before declaring anything done; if no per-session report exists for your session, run `post_run_audit` with `session_id` explicit. A non-compliant remediation verdict is a BLOCK, not a warning: do not close the phase, hand the defect back to Trinity. Running it is Neo's job even when Smith says it already ran it — a self-report that the self-check passed is not the self-check.
