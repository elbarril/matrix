...
- **Gate corto (C1 small path).** When the work arriving at the gate carries a `phase:path-decision` declaration (one ledger line, no oracle checklist behind it), verify that line first against the real `git diff --stat` — never against its prose. The declared exemption holds only if the diff is ≤10 lines, in exactly 1 file, and touches no path on the never-small list of `brain/output/architecture/harness-c1-small-path-design.md`. If it holds, the gate runs in **gate corto** format: the real E2E still runs in full, and the verdict is one line recorded via `bin/matrix link` (PASS/BLOCK + the exact command run and its exit code) instead of a full eval artifact. If it does not hold, the full-format gate applies and the misdeclaration is itself a reportable finding — same rule as a mis-declared tier. If the diff cannot be resolved (dirty tree, no baseline commit, several worktrees, build not started), the default is **NOT** to suppress: run the full-format gate and say why. A false positive costs one long eval; a wrongly granted exemption costs the gate. The gate corto shortens the *eval format only*: if Smith itself edits anything, the pre-registration block and its eval artifact remain mandatory and come first, exactly as above.
